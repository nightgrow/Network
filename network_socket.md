...
- 장점
  ...
  3.  TCP/IP 외의 프로토콜(데이터 통신을 원활하게 하기 위해 필요한 통신 규약)도 지원하므로 최소 코드 수정으로 응용 프로그램이 사용할 프로토콜을 변경할 수 있다.
...
- 송신에서 일방적으로 데이터를 전달하며, 수신에서 데이터를 받았는지 확인하지 않는다.
...
#### 특징
- 기본적으로 연결을 하지 않으므로 connect()함수를 사용하지 않는다.
- 데이터 경계 구분을 위한 작업을 애플리케이션이 하지 않아도 무관하다.
- 포로토콜 수준에서 신뢰성 있는 데이터 전송을 보장하지 않으므로, 필요 시 애플리케이션 수준에서 신뢰성 있는 데이터 전송 기능을 구현해야 한다.
- 간단한 소켓 함수 절차만 따르면 통신을 쉽게 구현할 수 있다.
- 1:1(unicast), 1:N(broadcast), N:N(multicast) 통신이 있다.

#### 동작
1. 서버는 소켓을 생성한 후 클라이언트가 데이터를 보내길 기다린다. 이때 서버가 사용하는 소켓은 특정 포트 번호와 결합되어(bind) 있어서 이 포트 번호로 보내는 데이터만 수신 가능하다.  
    ![1](https://t1.daumcdn.net/cfile/tistory/214B373A55349BAB04)
2. 클라이언트는 접속 과정 없이 바로 서버와 데이터를 주고 받는다.(sendto, recvfrom)  
    ![2](https://t1.daumcdn.net/cfile/tistory/220CEB4655349BAC09)
3. 또 다른 클라이언트 역시 접속 과정 없이 서버와 데이터를 주고 받는다.  
    ![3](https://t1.daumcdn.net/cfile/tistory/2411FF3555349BAC31)
4. 한 클라이언트가 두 개 이상의 소켓을 사용하여 서버와 통신하는 것도 가능하다.  
    ![4](https://t1.daumcdn.net/cfile/tistory/2152D73555349BAE0B)